...
static error_code ripstream_start_track_mp3 (RIP_MANAGER_INFO* rmi, 
					     TRACK_INFO* ti);
static error_code ripstream_start_track_ogg (RIP_MANAGER_INFO* rmi, 
					     TRACK_INFO* ti);
...
static error_code
ripstream_handle_ogg_bos (RIP_MANAGER_INFO* rmi, TRACK_INFO* ti);
...
ripstream_mp3_rip (RIP_MANAGER_INFO* rmi)
{
	...
	ret = cbuf3_init (&rmi->cbuf3, rmi->http_info.content_type, 
    ...
    ret = cbuf3_insert_chunk (rmi, &rmi->cbuf3, 
			      rmi->getbuffer, rmi->getbuffer_size,
			      rmi->http_info.content_type, 
			      &rmi->current_track);
    if (ret != SR_SUCCESS) {
	debug_printf("cbuf3_insert_chunk had bad return code %d\n", ret);
	return ret;
    }
	...
	ret = ripstream_start_track_mp3 (rmi, &rmi->current_track);
	if (ret != SR_SUCCESS) {
	...
	/* Start next track */
	ret = ripstream_start_track_mp3 (rmi, &rmi->new_track);
	if (ret != SR_SUCCESS)
	    real_ret = ret;
	rmi->find_silence = -1;
    ...
    if (cbuf3_get_free(&rmi->cbuf3) < rmi->getbuffer_size) {
	u_long curr_song;
	debug_printf ("cbuf3_get_free < getbuffer_size\n");
	extract_size = rmi->getbuffer_size - cbuf3_get_free(&rmi->cbuf3);
        ret = cbuf3_extract(rmi, &rmi->cbuf3, rmi->getbuffer, 
			    extract_size, &curr_song);
        if (ret != SR_SUCCESS) {
	    debug_printf("cbuf3_extract had bad return code %d\n", ret);
	    return ret;
	}
...
find_sep (RIP_MANAGER_INFO* rmi, u_long* pos1, u_long* pos2)
{
    ...
    rw_start = rmi->cbuf3.item_count - rmi->rw_start_to_cb_end;
    if (rw_start < 0) {
	return SR_ERROR_REQUIRED_WINDOW_EMPTY;
    }
    rw_end = rmi->cbuf3.item_count - rmi->rw_end_to_cb_end;
    if (rw_end < 0) {
	return SR_ERROR_REQUIRED_WINDOW_EMPTY;
    }
	...
	*pos2 = rw_start + sw_sil;
	...
	ret = cbuf3_peek_rgn (&rmi->cbuf3, buf, rw_start, bufsize);
	if (ret != SR_SUCCESS) {
...
// These positions are relative to cbuf3->read_index
static error_code
end_track_mp3 (RIP_MANAGER_INFO* rmi, u_long pos1, u_long pos2, TRACK_INFO* ti)
{
    ...
    ret = cbuf3_peek (&rmi->cbuf3, buf, pos1);
