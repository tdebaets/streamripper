     ...
     for (ch=0; ch<stereo; ch++) 
     {
       ...
       if(gr_info->big_values > 288) {
		   if (forceDecoding) {
				...
				errorstring = "big values too large";
				return 0;
		   }
	   }
       {
...
/* window-switching flag == 1 for block_Type != 0 .. and block-type == 0 -> win-sw-flag = 0 */
	   ...
	   if (gtkflag)
         ...
         if(gr_info->block_type == 0) {
			errorstring = "block_type == 0 && window_switching == 1 not allowed";
			 return 0;
         }
    ...
    for(i=0;i<2;i++) {
      ...
      for(;lp;lp--,mc--) {
        ...
        }
        {
          ...
          while((y=*val++)<0) {
			if (forceDecoding && part2remain == 0) break;
			if (get1bit())
              val -= y;
            part2remain--;
          }
        ...
        if(x == 15) {
          max[lwin] = cb;
		  if (forceDecoding && part2remain < h->linbits+1) break;
 ...
 */
  ...
  if(part2remain > 0)
	...
	errorstring = "can't rewind stream by ? bits";
...
#if 0
static void dct12(real *in,real *out1,real *out2,real *wp,real *ts)
{
  real    tmp[18];
  int i,j,p,m;
  ...
  for(i=0;i<3;i++)
    {
      for(p=0;p<12;p++){
	real sum = 0;
	for(m=0;m<6;m++) sum += in[i+3*m] * COS1[p][m];
	tmp[p] = sum * wp[p] ;
      }
	...
	case 0:
	  for(j=0;j<6;j++)
	    ts[j*SBLIMIT] = out1[j];
	  for(j=0;j<12;j++)
	    ts[(j+6)*SBLIMIT] = out1[j+6] + tmp[j];
	  break;
	case 1:
	  for(j=0;j<6;j++)
	    ts[(j+12)*SBLIMIT] += tmp[j];
	  for(j=0;j<6;j++)
	    out2[j] = tmp[j+6];
	  break;
	case 2:
...
int do_layer3(struct frame *fr,unsigned char *pcm_sample,int *pcm_point)
{
  ...
  if(fr->lsf) {
    granules = 1;
    if (!III_get_side_info_2(&sideinfo,stereo,ms_stereo,sfreq,single)) return -1;
  }
  else {
    granules = 2;
#ifdef MPEG1
    if (!III_get_side_info_1(&sideinfo,stereo,ms_stereo,sfreq,single)) return -1;
#else
