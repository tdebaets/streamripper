		...
		int osize,int *done)
{
	...
	if(mp->framesize == 0) {
		...
		totalFramesize += mp->framesize;
		{
			...
			currentBitrate = tabsel_123[mp->fr.lsf][2][mp->fr.bitrate_index];
		}
		nframe++;
	}
	...
	*done = 0;
	if(mp->fr.error_protection)
           getbits(16);
	if (do_layer3(&mp->fr,(unsigned char *) out,done) == -1) return MP3_ERR;
