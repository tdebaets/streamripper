...
#
#   GCS: 2007/5/20	Add comments for autoheader template
#
AC_DEFUN([MP_WITH_CURSES],
  [AC_ARG_WITH(ncurses, [  --with-ncurses          Force the use of ncurses over curses],,)
   ...
   then
     ...
     then
       AC_DEFINE(HAVE_CURSES_H,0,[Define to 1 if you have curses.h])
       CURSES_LIB="-lcurses"
     fi
   fi
   if test ! "$CURSES_LIB"
   then
     ...
     then
       AC_DEFINE(HAVE_NCURSES_H,0,[Define to 1 if you have ncurses.h])
       CURSES_LIB="-lncurses"
     fi
   fi
   LIBS="$mp_save_LIBS"
